...
app.get('/:repo/demo-frames', (req, res) => {
	...
	files = files.map((file) => {
		...
		// Check for asset config and add it if needed
		let assetConfigPath = `${repoDir}/${path}/assets/asset-config.json`;
		if (fs.existsSync(assetConfigPath)) {
			let options = { encoding: 'utf8' };
			let assetConfig = JSON.parse(fs.readFileSync(assetConfigPath, options));
			demoFrame.asset = assetConfig;
		}
		return demoFrame;
	});
	// Send the files back
